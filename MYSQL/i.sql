...
SELECT COUNT(animal_id)AS total_animal FROM zoo_animal
WHERE age_years >5
...
SELECT AVG(age_years)AS total_weight FROM zoo_animal

SELECT
       COUNT(animal_id)AS total_animal,
       COUNT(DISTINCT species)AS unique_species ,
       SUM(weight_kg)AS total_weight,
       AVG(age_years)AS total_weight FROM zoo_animal;

SELECT * FROM zoo_animal
WHERE animal_id BETWEEN 6 AND 7